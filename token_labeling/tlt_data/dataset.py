...
from .memcache import MemCachedLoader
...
class DatasetTokenLabel(data.Dataset):
    ...
    def __init__(
        ...
        self.mc_initialized = False

    def _init_memcached(self):
        if not self.mc_initialized:
            assert self.mc_config_path is not None
            self.mc_loader = MemCachedLoader(self.mc_config_path, tcs=True)
            self.mc_initialized = True
    ...
    def __getitem__(self, index):
        ...
        if self.use_memcache:
            self._init_memcached() # TODO: why not put into __init__?
            score_maps = self.mc_loader(score_path, load_type='torch_tensor')
...
def create_token_label_dataset(dataset_type, root, label_root, use_memcache=False, mc_config_path=None):
    ...
    return DatasetTokenLabel(train_dir, label_root,
        use_memcache=use_memcache, mc_config_path=mc_config_path)