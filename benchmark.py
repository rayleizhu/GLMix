...
copied from https://github.com/huggingface/pytorch-image-models/blob/main/benchmark.py
"""
import argparse
import csv
import json
...
# sda kernel 
parser.add_argument('--sdpa-kernel', type=str, default='math',
                    choices=['math', 'memeff', 'flash'],
                    help='scaled dot product kernel choice.')
...
def benchmark(args):
    ...
    torch_ver_major = int(str.split(torch.__version__, '.')[0])
    if args.sdpa_kernel == 'math':
        sdpa_kwargs = {"enable_math": True, "enable_flash": False, "enable_mem_efficient": False}
    elif args.sdpa_kernel == 'flash':
        assert torch_ver_major >= 2, "flash attention kernel requires torch >= 2.0"
        sdpa_kwargs = {"enable_math": False, "enable_flash": True, "enable_mem_efficient": False}
    elif args.sdpa_kernel == 'memeff':
        assert torch_ver_major >= 2, "memory efficient attention kernel requires torch >= 2.0"
        sdpa_kwargs = {"enable_math": False, "enable_flash": False, "enable_mem_efficient": True}
    ...
    for prefix, bench_fn in zip(prefixes, bench_fns):
        if torch_ver_major >= 2:
            with torch.backends.cuda.sdp_kernel(**sdpa_kwargs):
                _logger.info(f'benchmark with spda_kwargs={sdpa_kwargs}')
